...

{
	...
	if (((which & SSRF_RENDERFLOOR) && (rt_nocull_flat || frontsector->floorplane.ZatPoint(vp.Pos) <= vp.Pos.Z) && (!section || !(section->flags & FSection::DONTRENDERFLOOR))) && !(vp.IsOrtho() && (vp.PitchSin < 0.0)))
	...
	if (((which & SSRF_RENDERCEILING) && (rt_nocull_flat || frontsector->ceilingplane.ZatPoint(vp.Pos) >= vp.Pos.Z) && (!section || !(section->flags & FSection::DONTRENDERCEILING))) && !(vp.IsOrtho() && (vp.PitchSin > 0.0)))
