...
#include "common/rendering/rt/rt_state.h"
...
namespace
{
	...
	static bool GetLocalControllerPose(int abstractHand, DVector3& pos)
	{
		...
		const FLOATTYPE* matrix = mat.get();
		pos = DVector3(matrix[12], matrix[13], matrix[14]);
		return true;
	}
	...
	static void PlayWheelHaptics(const VRMode* vrmode, int hand, float intensity)
	{
		...
		RT_OpenXRInputHaptic(hapticChannel, 0.035f, intensity);
	}
	...
	static void DrawWorldQuad(HWDrawInfo* di, FRenderState& state, const DVector3& center, const DVector3& right, const DVector3& up, float width, float height, FGameTexture* texture, PalEntry color, bool textured, bool rotate180 = false)
	{
		...
		screen->mVertexData->Unmap();
		#if HAVE_RT
		// RT must preserve the icon texture alpha; otherwise its transparent pixels become black.
		auto rtWheelIconPrim = rtstate.push_type(RtPrim::Particle);
		auto rtWheelIconBrightness = rtstate.push_type(RtPrim::WheelIcon);
#endif
...
void VRWheel_Draw(HWDrawInfo* di, FRenderState& state)
{
	...
	if (menuactive != MENU_Off || ConsoleState != c_up || (false))
	{
		return;
	}
...
#if HAVE_RT
	// The wheel is a world-anchored HUD surface: keep it full-bright and unaffected by map lighting.
	auto rtWheelPrim = rtstate.push_type(RtPrim::Wheel);
#endif
	...
	if (!UseCinemaWheelOverride() && vr_wheel_selection_type == 0)
	{
		...
		{
#if HAVE_RT
			auto rtTouchId = rtstate.push_uniqueid(&GVRWheel, 0x575245454C01ULL);
#endif
	...
	else if (layout.RingCount > 1)
	{
		...
		#if HAVE_RT
		{ auto rtOuterId = rtstate.push_uniqueid(&GVRWheel, 0x575245454C02ULL);
#endif
		DrawWorldDisc(di, state, center, wheelRight, wheelUp, outerIndicatorRadius, outerColor);
#if HAVE_RT
		}
		{ auto rtInnerId = rtstate.push_uniqueid(&GVRWheel, 0x575245454C03ULL);
#endif
		DrawWorldDisc(di, state, center, wheelRight, wheelUp, outerIndicatorInnerRadius, bgColor);
#if HAVE_RT
		}
		{ auto rtCenterId = rtstate.push_uniqueid(&GVRWheel, 0x575245454C04ULL);
#endif
	...
	for (int i = 0; i < count; ++i)
	{
		...
		// Each icon is a separate RT object; sharing one identity corrupts its mesh history.

		auto rtIconId = rtstate.push_uniqueid(&GVRWheel, 0x575245455000ULL + uint64_t(i));

		#endif

		DrawWorldDisc(di, state, iconCenter + wheelForward * 0.10, wheelRight, wheelUp, backdropSize * 0.50f, backdropColor);
