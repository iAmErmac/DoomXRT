...
static DVector3 GetLocalTracerOrigin(AActor* actor, int flags, const DVector3& fallback)
{
	...
	// DoomXRT OpenXR transforms are already in game coordinates, so no Y/Z swap is required.
	return DVector3(controllerMatrix[12], controllerMatrix[13], controllerMatrix[14]);
}
